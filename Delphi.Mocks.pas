...
type
  ...
  IStubSetup<T> = interface
    ...
    //set return values for a method, one for each call, when called with the parameters specified on the When
    function WillReturnInSequence(const values : TArray<TValue>) : IWhen<T>; overload;

    //set return values for a method, one for each call, no matter what parameters are specified.
